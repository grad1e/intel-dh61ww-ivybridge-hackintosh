...
* [SSDTTime](https://github.com/corpnewt/SSDTTime) - Script for generating SSDT-EC.aml which is necessary for macOS Catalina.
...
* Map the USB ports using USBMap (link above) after successful installation of macOS. Verify that the USB power is also working as intended (can be identified using an option in USBMap script). After mapping the ports, either remove or disable the 'Port Limit Increase' patch from `config.plist -> KernelAndKextPatches -> KextsToPatch`.
...
I've enabled TRIM via config.plist so it should work. If it is not, enable it by executing `sudo trimforce enable`
* Generate SSDT-EC.aml using SSDTTime (link above) by following these steps if you've already installed macOS Mojave or High Sierra. SSDT-EC.aml can also be generated in Windows (IDK, find it out)
	* Dump your DSDT by pressing F4 in the clover boot menu
	* DSDT.aml can be found in `EFI -> Clover -> ACPI -> origin`
	* Use the DSDT.aml in SSDTTime script to generate the required SSDT-EC.aml and place it in `EFI -> Clover -> ACPI -> patched`
